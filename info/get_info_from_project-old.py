...
If the images were uploaded to the project with the following format 'tissue_original_name' 
    ...
    with Cytomine(host=params.host, public_key=params.public_key, private_key=params.private_key,
        ...
        with open('./info/project-info.csv', mode='w') as csv_file:
            ...
            for image in image_instances:
                ...
                image.filename : /1550699772134/lung-lesion_2_29-041-Izd2-w35-Ki67-7-les2_pyr.tif
                ...
                # image.filename : /1550699772134/lung-lesion_2_29-041-Izd2-w35-Ki67-7-les2_pyr.tif
                # lung-lesion_2_29-041-Izd2-w35-Ki67-7-les2_pyr.tif (remove the path)
                tmp = image.filename.split('/')[2]
                # lung-lesion_2_29-041-Izd2-w35-Ki67-7-les2_pyr (remove the '.tif')
                tmp = tmp.split('.')[0]
                # lung-lesion_2_29-041-Izd2-w35-Ki67-7-les2 (remove the '_pyr')
                tmp = tmp.rsplit('_', 1)[0]
                tmp = tmp.split("_")

                # e.g. original_name -> 29-041-Izd2-w35-Ki67-7-les2
                original_name = "_".join(tmp[2:])
                # e.g. tissue -> lung-lesion_2
                tissue = '_'.join([tmp[0], tmp[1]])
                ...
                # Get the scale of the image (the name of folder containing the images)

                scale = ""
                # Get the path to the reference image
                images_path = params.path_to_images + "/" + tissue
                ...
                scale = entries[0]
                original_reference_filepath = images_path + "/" + \
                    scale + "/" + original_name
